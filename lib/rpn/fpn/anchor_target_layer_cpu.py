...
def anchor_target_layer(rpn_cls_score_list, gt_boxes, im_info, _feat_stride, all_anchors, num_anchors):
  ...
  '''For all RPN experiments (including baselines), we include the 
  anchor boxes that are outside the image for training, which
  is unlike [29] where these anchor boxes are ignored. 
  '''
  ...
  if cfg.TRAIN.RPN_POSITIVE_WEIGHT < 0:
    ...
    assert num_examples == cfg.TRAIN.RPN_BATCHSIZE
