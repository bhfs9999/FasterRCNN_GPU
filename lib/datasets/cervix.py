...
class cervix_data(imdb):
  ...
  def _get_image_type_name(self):
    if self._image_type == '_1':
      return 'salt'
    elif self._image_type == '_2':
      return 'acid'
    elif self._image_type == '':
      return 'other'
    else:
      print('image_type must in salt, acid')
      raise TypeError
  ...
  def image_path_from_index(self, index):
    ...
    """
    image_path = os.path.join(self._data_path, 'pic',
  ...
  def _load_image_set_index(self):
    ...
    image_set_file = os.path.join(self._data_path, 'data_split', self._image_type_name, self._image_set + '.txt')
    assert os.path.exists(image_set_file), \
      'Path does not exist: {}'.format(image_set_file)
    with open(image_set_file) as f:
      image_index = [x.strip() + self._image_type for x in f.readlines()]
    return image_index
