...
from lib.model.eval import SolverWrapper as EvaluateSolverWrapper
try:
...
class SolverWrapper(object):
  def __init__(self, network, imdb, valimdb, roidb, valroidb, model_dir, pretrained_model=None):
    self.net = network
    self.imdb = imdb
    ...
    for step in range(self.start_step, max_iters + 1):
      ...
      if self._evaluate:
        if step > self._begin_eval_point and step % cfg.TRAIN.EVALUATE_POINT == 0:
          self.net.eval()
          evaluate_solverwrapper = EvaluateSolverWrapper(network=self.net, imdb=self.valimdb,
                                                              model_dir=None, output_dir=self.evaluate_dir)
          ...
          if self._use_tensorboard:
            self._tensor_writer.add_scalars('metrics_cls', metrics_cls, global_step=step)
            self._tensor_writer.add_scalars('metrics_reg', metrics_reg, global_step=step)
...
def train_net(network, imdb, valimdb, roidb, valroidb, model_dir,
              learn_dict, resume,
  ...
  sw = SolverWrapper(network, imdb, valimdb, roidb, valroidb, model_dir,
