...
class RPN(nn.Module):
  ...
  def forward(self, im_data, im_info, gt_boxes=None):
    ...
    if self.training:
      ...
      self.cross_entropy, self.loss_box = self._build_loss(rpn_cls_score_list, rpn_bbox_score_list, rpn_data)
  ...
  def _build_loss(self, rpn_cls_score_list, rpn_bbox_score_list, rpn_data, sigma_rpn=3):
    rpn_cls_score = [self._reshape_loss(rpn_cls_score_reshape, 2).permute(0, 3, 4, 2, 1).contiguous().view(-1, 2)
                     for rpn_cls_score_reshape in rpn_cls_score_list]
  ...
  def rois_split_level(rois):
    def calc_level(width, height):
      ...
      return min(6, max(2, int(round(inner, 0))))
