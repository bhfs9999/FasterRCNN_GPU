...
class RPN(nn.Module):
  ...
  def _build_loss(self, rpn_cls_score_reshape, rpn_bbox_score, rpn_data, sigma_rpn=3):
    ...
    #   rpn_fg = Variable(rpn_label.data.eq(1).nonzero().squeeze())
    #   rpn_bg = Variable(rpn_label.data.eq(0).nonzero().squeeze())
    #   print(rpn_fg.numel(), rpn_bg.numel())
    #   print(rpn_keep.numel())
...
class FasterRCNN(nn.Module):
  ...
  def _predict(self, im_data, im_info, gt_boxes):
    ...
    if self.training:
      # 根据阈值筛选RPN提取出的rois，选出batch_size大小数量的rois，确保正负比例，并计算其标签，回归参数，loss中的权重，用于训练
  ...
  def train_operation(self, blobs, optimizer, image_if=False, clip_parameters=None):
    ...
    image = None
    if image_if:
      # 每隔固定step，储存一次图像，gt用红框，预测框用其他颜色
